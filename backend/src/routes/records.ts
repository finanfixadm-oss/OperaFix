...
recordsRouter.delete("/bulk/delete", async (req, res) => {
  try {
    ...
    for (const recordId of ids) {
      ...
      results.push({ id: recordId, management: managementDeleted.count, legacy: legacyDeleted, documents: docsDeleted });
    }

    res.json({ ok: true, requested: ids.length, deleted: results.filter((item) => item.management || item.legacy).length, results });
  } catch (error: any) {
    console.error("Error en eliminación masiva:", error);
    res.status(500).json({ message: "No se pudo ejecutar la eliminación masiva.", detail: error?.message || String(error) });
  }
});
