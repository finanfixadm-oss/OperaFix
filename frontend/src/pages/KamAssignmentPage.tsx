...
export default function KamAssignmentPage() {
  ...
  const canAdmin = ["admin", "kam_admin", "interno"].includes(
    String(user?.role || ""),
  );
  const canConfigure = ["admin", "kam_admin"].includes(
    String(user?.role || ""),
  );
  ...
  const [draggingCompanyId, setDraggingCompanyId] = useState<string | null>(
    null,
  );
  ...
  async function loadAll() {
    ...
    try {
      ...
      const campaignPromise = canAdmin
        ? fetchJson<KamCampaign[]>("/kam/campaigns")
        : Promise.resolve([] as KamCampaign[]);
      const agendaPromise = fetchJson<AgendaRow[]>("/kam/agenda").catch(
        () => [] as AgendaRow[],
      );
      ...
      if (canConfigure) {
        const userRows = await fetchJson<UserRow[]>("/kam/users").catch(
          () => [] as UserRow[],
        );
        setUsers(
          userRows.filter(
            (item) =>
              String(item.role).toLowerCase() === "kam" &&
              item.active !== false,
          ),
        );
      }
  ...
  useEffect(() => {
    const requested = new URLSearchParams(location.search).get("tab") as
      | typeof tab
      | null;
    if (
      requested &&
  ...
  const filterOptions = useMemo(
    () => ({
      kams: uniqueSorted(
        companies.map(
          (x) =>
            x.kam_asignado_nombre || (!x.kam_asignado_id ? "Sin asignar" : ""),
        ),
      ),
  ...
  const filteredCompanies = useMemo(() => {
    ...
    return companies.filter((row) => {
      const assignedName =
        row.kam_asignado_nombre || (!row.kam_asignado_id ? "Sin asignar" : "");
      const quickOk =
        !filterQuick ||
        ...
        (filterQuick === "sin_contacto" &&
          !row.fecha_ultimo_contacto &&
          Boolean(row.kam_asignado_id)) ||
      ...
      const qOk =
        ...
        [
          ...
          row.estado,
          row.prioridad,
          row.kam_asignado_nombre,
          row.correo,
          row.telefono,
          row.telefono_central,
          row.linkedin_url,
        ]
          .filter(Boolean)
      ...
      return (
        ...
        (!filterCampaign ||
          row.campaign_nombre === filterCampaign ||
          row.campaign_id === filterCampaign) &&
        quickOk
      );
    });
  }, [
  ...
  const dashboard = useMemo(() => {
    ...
    const amount = (items: KamCompany[]) =>
      items.reduce((sum, item) => sum + Number(item.monto_devolucion || 0), 0);
    const total = rows.length;
    const sinAsignar = rows.filter((x) => !x.kam_asignado_id).length;
    const enGestion = rows.filter((x) =>
      ESTADOS_GESTION.includes(String(x.estado || "")),
    ).length;
    ...
    const sinContacto = rows.filter(
      (x) => x.kam_asignado_id && !x.fecha_ultimo_contacto,
    ).length;
    const montoPotencial = amount(rows);
    const montoGanado = amount(rows.filter((x) => x.estado === "Ganada"));
    const probPromedio = rows.length
      ? rows.reduce((sum, x) => sum + Number(x.probabilidad_cierre || 0), 0) /
        rows.length
      : 0;
    const byEstado = ESTADOS_VENTA.map((estado) => {
      const items = rows.filter(
        (x) => String(x.estado || "Sin asignar") === estado,
      );
      return { estado, total: items.length, monto: amount(items) };
    }).filter((x) => x.total > 0);
    const byKam = Array.from(
      new Set(
        rows.map(
          (x) =>
            x.kam_asignado_nombre ||
            (!x.kam_asignado_id ? "Sin asignar" : "KAM sin nombre"),
        ),
      ),
    )
      .sort((a, b) => a.localeCompare(b, "es"))
      .map((kam) => {
        const items = rows.filter(
          (x) =>
            (x.kam_asignado_nombre ||
              (!x.kam_asignado_id ? "Sin asignar" : "KAM sin nombre")) === kam,
        );
        ...
        const prob = items.length
          ? items.reduce(
              (sum, x) => sum + Number(x.probabilidad_cierre || 0),
              0,
            ) / items.length
          : 0;
  ...
  const kpis = useMemo(() => {
    const strategic = companies.filter(
      (x) => x.prioridad === "Estratégica",
    ).length;
    const unassigned = companies.filter((x) => !x.kam_asignado_id).length;
    const totalAmount = companies.reduce(
      (sum, x) => sum + Number(x.monto_devolucion || 0),
      0,
    );
  ...
  async function editCompany(row: KamCompany) {
    ...
    const rows = await fetchJson<KamActivity[]>(
      `/kam/companies/${row.id}/activities`,
    ).catch(() => [] as KamActivity[]);
    setActivities(rows);
    const contactRows = await fetchJson<KamContact[]>(
      `/kam/companies/${row.id}/contacts`,
    ).catch(() => [] as KamContact[]);
  ...
  async function openActivityModal(row?: KamCompany) {
    ...
    if (!selected || selected.id !== target.id) {
      ...
      setCompanyForm({
        ...
        proxima_gestion: target.proxima_gestion
          ? String(target.proxima_gestion).slice(0, 10)
          : "",
      });
      const rows = await fetchJson<KamActivity[]>(
        `/kam/companies/${target.id}/activities`,
      ).catch(() => [] as KamActivity[]);
      setActivities(rows);
    }
    setEditingActivityId(null);
  ...
  async function saveCompany() {
    ...
    if (!selected) {
      const rutNorm = String(companyForm.rut || "")
        .toUpperCase()
        .replace(/[^0-9K]/g, "");
      const razonNorm = String(companyForm.razon_social || "")
        .trim()
        .toLowerCase();
      const exists = companies.some((item) => {
        const itemRut = String(item.rut || "")
          .toUpperCase()
          .replace(/[^0-9K]/g, "");
        const itemRazon = String(item.razon_social || "")
          .trim()
          .toLowerCase();
        return (
          (!!rutNorm && itemRut === rutNorm) ||
          (!!razonNorm && itemRazon === razonNorm)
        );
      });
      if (exists) {
        notifyError(
  ...
  async function saveActivity() {
    ...
    if (
      activityForm.estado_venta === "Perdida" &&
  ...
  async function refreshContacts(companyId?: string) {
    ...
    if (!id) return;
    const rows = await fetchJson<KamContact[]>(
      `/kam/companies/${id}/contacts`,
    ).catch(() => [] as KamContact[]);
    setContacts(rows);
  }
  ...
  function addDraftContact() {
    ...
    setDraftContacts((prev) => {
      const cleaned = next.es_principal
        ? prev.map((c) => ({ ...c, es_principal: false }))
        : prev;
      return [...cleaned, next];
    });
  ...
  function promoteDraftContact(index: number) {
    setDraftContacts((prev) =>
      prev.map((c, idx) => ({ ...c, es_principal: idx === index })),
    );
  ...
  async function saveContact() {
    ...
    try {
      if (editingContactId)
        await putJson(`/kam/contacts/${editingContactId}`, contactForm);
      else
        await postJson(`/kam/companies/${selected.id}/contacts`, contactForm);
      setContactForm(emptyContact);
      setEditingContactId(null);
  ...
  async function deleteCompany(row: KamCompany) {
    ...
    if (
      !window.confirm(
        `¿Eliminar la empresa ${row.razon_social}? Esta acción eliminará también contactos, bitácora e historial KAM.`,
      )
    )
      return;
    try {
  ...
  async function recommend(row: KamCompany) {
    ...
    const result = await fetchJson<{ recommendations: KamProfile[] }>(
      `/kam/recommend/${row.id}`,
    );
  ...
  async function exportCompanies() {
    await downloadBlob(
      "/kam/companies/export-excel",
      {
        ...
        campaign_id:
          filterCampaign &&
          campaigns.find((c) => c.nombre === filterCampaign)?.id
            ? campaigns.find((c) => c.nombre === filterCampaign)?.id
            : undefined,
  ...
  async function moveKanbanCard(row: KamCompany, nuevoEstado: string) {
    ...
    if (nuevoEstado === "Perdida" && !motivo_perdida) {
      const motivo = window.prompt(
        "Indica el motivo de pérdida para mover esta empresa a Perdida:",
      );
    ...
    const proximaGestion = row.proxima_gestion
      ? String(row.proxima_gestion).slice(0, 10)
      : row.kam_asignado_id &&
          !["Ganada", "Perdida", "Congelada"].includes(nuevoEstado)
        ? tomorrowDate()
        : "";
        ...
        prev.map((item) =>
          item.id === row.id
            ? {
                ...
                estado: nuevoEstado,
                motivo_perdida,
                proxima_gestion: proximaGestion || item.proxima_gestion,
              }
            : item,
        ),
      );
  ...
  function estadoClass(value?: string | null) {
    const key = String(value || "sin-asignar")
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/[^a-z0-9]+/g, "-");
    ...
    <div className="zoho-module-page kam-module-page kam-visual-pro">
      ...
      <div className="zoho-kpi-grid kam-pro-kpis">
        <div className="zoho-card kam-kpi-card">
          <span className="kam-kpi-icon">🏢</span>
          <strong>{metrics.summary?.total_empresas ?? kpis.total}</strong>
          <span>Empresas potenciales</span>
        </div>
        <div className="zoho-card kam-kpi-card success">
          <span className="kam-kpi-icon">✅</span>
          <strong>{metrics.summary?.ganadas ?? 0}</strong>
          <span>Ganadas</span>
        </div>
        <div className="zoho-card kam-kpi-card warning">
          <span className="kam-kpi-icon">📌</span>
          <strong>{metrics.summary?.sin_asignar ?? kpis.unassigned}</strong>
          <span>Sin asignar</span>
        </div>
        <div className="zoho-card kam-kpi-card money">
          <span className="kam-kpi-icon">💰</span>
          <strong>
            {money(metrics.summary?.monto_potencial ?? kpis.totalAmount)}
          </strong>
      ...
      {saveMessage && (
        <div className="zoho-card kam-save-message">
      ...
      <div className="kam-workspace-main kam-workspace-main-full">
        <section className="zoho-card kam-dashboard-card kam-filter-card">
          ...
          <div className="kam-filter-shell">
            ...
            {filtersExpanded && (
              <div className="kam-zoho-filter-panel">
                <div className="kam-zoho-filter-head">
                  <span>Campo</span>
                  <span>Criterio / valor</span>
                  <span>Acción</span>
                </div>
                {activeFilterPills.map((item) => (
                  <div key={item.key} className="kam-zoho-filter-row">
                    <div className="kam-zoho-filter-name">
                      <span className="kam-filter-number">•</span>
                      <strong>{item.label}</strong>
                    </div>
                    <div className="kam-zoho-filter-control">
                      {item.key === "kam" && (
                        <select
                          className="zoho-input"
                          value={filterKam}
                ...
                {!activeFilterPills.length && (
                  <div className="kam-zoho-filter-empty">
                    No tienes filtros avanzados activos. Agrega uno desde el selector superior.
                  </div>
                )}
              </div>
            )}
                ...
                (selected ? (
                  <div className="zoho-form-grid single-column-form">
                    <p className="zoho-help-text">
                      Actualiza el seguimiento de la empresa asignada. Solo ves
                      empresas de tu cartera.
                    </p>
                    <Field label="Empresa">
                      <input
                        className="zoho-input"
                    ...
                    <Field label="Estado">
                      ...
                      >
                        {ESTADOS_VENTA.filter((x) => x !== "Sin asignar").map(
                          (x) => (
                  ...
                  <p className="zoho-help-text">
                    Selecciona una empresa de tu cartera para actualizar el
                    seguimiento.
                  </p>
                ))}
              {canAdmin && (
                <div className="zoho-form-grid single-column-form">
                  <Field label="RUT">
                    <input
                      ...
                      onChange={(e) =>
                        setCompanyForm({ ...companyForm, rut: e.target.value })
                      }
                    />
                  </Field>
                  ...
                  <Field label="Nro Empleados">
                    <input
                      className="zoho-input"
                      type="number"
                      value={companyForm.nro_empleados}
                      onChange={(e) =>
                        setCompanyForm({
                          ...companyForm,
                          nro_empleados: e.target.value,
                        })
                      }
                    />
                  </Field>
                  <Field label="Monto Devolución">
                    <input
                      className="zoho-input"
                      type="number"
                      value={companyForm.monto_devolucion}
                      onChange={(e) =>
                        setCompanyForm({
                          ...companyForm,
                          monto_devolucion: e.target.value,
                        })
                      }
                    />
                  </Field>
                  ...
                  <Field label="Nombre contacto">
                    <input
                      className="zoho-input"
                      value={companyForm.nombre_contacto}
                      onChange={(e) =>
                        setCompanyForm({
                          ...companyForm,
                  ...
                  <Field label="Cargo">
                    <input
                      className="zoho-input"
                      value={companyForm.cargo_contacto}
                      onChange={(e) =>
                        setCompanyForm({
                          ...companyForm,
                          cargo_contacto: e.target.value,
                        })
                      }
                    />
                  </Field>
                  ...
                  {!selected && (
                    <div className="kam-contact-builder">
                      <div className="kam-section-title">
                        ...
                        <span>
                          Puedes crear 1, 2, 3 o más contactos antes de guardar
                          la empresa.
                        </span>
                      </div>
                          ...
                          {draftContacts.map((c, idx) => (
                            ...
                            >
                              ...
                              <div className="zoho-actions-row compact-actions">
                                ...
                                {c.es_principal && (
                                  <span className="kam-badge success">
                                    Principal
                                  </span>
                                )}
                                <button
            ...
            <section className="zoho-table-wrap">
                  ...
                  <tbody>
                    {filteredCompanies.map((row) => (
                      <tr key={row.id}>
                        ...
                        <td>
                          {row.source_mandante_name || "Finanfix Solutions SPA"}
                        </td>
                        ...
                        <td>
                          <span className={estadoClass(row.estado)}>
                            {row.estado || "—"}
                          </span>
                        </td>
                        <td>
                        ...
                        <td>
                          <span
                            className={`priority-chip priority-${String(row.prioridad || "").toLowerCase()}`}
                          >
                            {row.prioridad || "—"}
                          </span>
                        </td>
                    ...
                    {!filteredCompanies.length && (
                      <tr>
                        <td colSpan={22}>Sin empresas KAM cargadas.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              {selected && (
                <>
                  <div className="zoho-card kam-recommend-box">
                    ...
                    <p className="zoho-help-text">
                      Agrega más de un contacto por empresa, con teléfono
                      directo, correo y LinkedIn. Puedes marcar un contacto como
                      principal para actualizar la ficha comercial.
                    </p>
                    <div className="zoho-form-grid kam-filter-grid">
                      ...
                      <label className="zoho-form-field">
                        <span>Principal</span>
                        <label className="kam-checkbox-line">
                          <input
                            type="checkbox"
                  ...
                  <div className="zoho-card kam-recommend-box">
                    ...
                    <p className="zoho-help-text">
                      Registra llamadas, correos, reuniones, próximas acciones y
                      cambios de estado para no perder trazabilidad.
                    </p>
                    <div className="zoho-form-grid kam-filter-grid">
                      <Field label="Tipo gestión">
                        <select
                          className="zoho-input"
                          value={activityForm.tipo_gestion}
                          onChange={(e) =>
                            setActivityForm({
                              ...activityForm,
                              tipo_gestion: e.target.value,
                            })
                          }
                        >
                      ...
                      <Field label="Estado venta">
                        <select
                          className="zoho-input"
                          value={activityForm.estado_venta}
                          onChange={(e) =>
                            setActivityForm({
                              ...activityForm,
                      ...
                      <Field label="Resultado">
                        <input
                          ...
                          placeholder="Contactado, sin respuesta, interesado..."
                        />
                      </Field>
                      <Field label="Próxima acción">
                        <input
                          className="zoho-input"
                          value={activityForm.proxima_accion}
                          onChange={(e) =>
                            setActivityForm({
                              ...activityForm,
                      ...
                      <Field label="Próxima gestión">
                        <input
                          className="zoho-input"
                          type="date"
                          value={activityForm.proxima_gestion}
                          onChange={(e) =>
                            setActivityForm({
                              ...activityForm,
                              proxima_gestion: e.target.value,
                      ...
                      <Field label="Probabilidad %">
                        <input
                          ...
                          min={0}
                          max={100}
                          value={activityForm.probabilidad_cierre}
                          onChange={(e) =>
                            setActivityForm({
                              ...activityForm,
                              probabilidad_cierre: e.target.value,
                      ...
                      <Field label="Observación">
                        <textarea
                          className="zoho-input"
                          rows={2}
                          value={activityForm.observacion}
                          onChange={(e) =>
                            setActivityForm({
                              ...activityForm,
                              observacion: e.target.value,
                        ...
                        <tbody>
                          {activities.map((a) => (
                            <tr key={a.id}>
                              ...
                              <td>{a.estado_venta || "—"}</td>
                              <td>{a.resultado || "—"}</td>
                              <td>{a.proxima_accion || "—"}</td>
                              <td>
                                {a.proxima_gestion
                                  ? String(a.proxima_gestion).slice(0, 10)
                                  : "—"}
                              </td>
              ...
              {recommendations.length > 0 && selected && (
                <div className="zoho-card kam-recommend-box">
                  ...
                  <p>
                    El sistema compara rubro, región, tipo de oportunidad,
                    tamaño, monto potencial, carga actual y ranking comercial.
                  </p>
                  <table className="zoho-table">
                    <thead>
                      <tr>
                        <th>KAM recomendado</th>
                        <th>Nivel</th>
                        <th>Ranking</th>
                        <th>Carga</th>
                        <th>Score match</th>
                    ...
                    <tbody>
                      {recommendations.slice(0, 5).map((profile) => (
                        <tr key={profile.user_id}>
                          ...
                          <td>
                            {profile.carga_actual || 0}/
                            {profile.capacidad_maxima}
                          </td>
                          <td>
                            <strong>{profile.score_match}</strong>
                          </td>
                          <td>
                            {(profile.motivos || []).join(" · ") ||
                              "Compatibilidad general"}
                          </td>
                          <td>
                            <button
                              className="zoho-small-btn primary"
                              onClick={() => assign(selected, profile)}
                            >
                              Asignar
                            </button>
                          </td>
        ...
        {tab === "kanban" && (
          <section className="zoho-dashboard-stack">
            <div className="zoho-table-toolbar">
              ...
              <span className="zoho-help-text">
                Arrastra una tarjeta a otra columna para cambiar automáticamente
                el estado de venta. El cambio queda registrado en la bitácora.
              </span>
            </div>
            <div className="kam-kanban-board">
              {ESTADOS_VENTA.filter((estado) => estado !== "Sin asignar").map(
                (estado) => {
                  const items = filteredCompanies
                    .filter((row) => String(row.estado || "") === estado)
                    .slice(0, 50);
                    ...
                    >
                      ...
                      {items.map((row) => (
                        ...
                        >
                          ...
                          <span>
                            {row.kam_asignado_nombre || "Sin asignar"}
                          </span>
                          <span>
                            {money(row.monto_devolucion)} ·{" "}
                            {row.nro_empleados || "—"} trab.
                          </span>
                          <span>
                            {row.rubro || "Sin rubro"} ·{" "}
                          ...
                          {kanbanUpdatingId === row.id && (
                            <span className="kam-saving-text">
                              Actualizando estado...
                            </span>
                          )}
                        </button>
                      ...
                      {!items.length && (
                        <p className="zoho-help-text">
                          Suelta aquí una tarjeta para cambiarla a este estado.
                        </p>
                      )}
                    </div>
                  );
        ...
        {tab === "tracking" && (
          <section className="zoho-dashboard-stack">
            <div className="zoho-kpi-grid">
              ...
              <div className="zoho-card">
                <strong>{dashboard.enGestion}</strong>
                <span>En gestión</span>
              </div>
              <div className="zoho-card">
                <strong>{dashboard.ganadas}</strong>
                <span>Ganadas</span>
              </div>
              <div className="zoho-card">
                <strong>{dashboard.perdidas}</strong>
                <span>Perdidas</span>
              </div>
              <div className="zoho-card">
                <strong>{dashboard.vencidas}</strong>
                <span>Gestiones vencidas</span>
              </div>
              <div className="zoho-card">
                <strong>{dashboard.sinContacto}</strong>
                <span>Sin primer contacto</span>
              </div>
              <div className="zoho-card">
                <strong>
                  {Number(dashboard.probPromedio || 0).toFixed(0)}%
                </strong>
                <span>Probabilidad promedio</span>
              </div>
              <div className="zoho-card">
                <strong>{money(dashboard.montoGanado)}</strong>
            ...
            <section className="zoho-table-wrap">
              <div className="zoho-table-toolbar">
                <span>Embudo de ventas por estado</span>
                <span className="zoho-help-text">
                  Muestra cantidad de empresas y monto potencial según los
                  filtros activos.
                </span>
              </div>
              <div className="zoho-table-scroll-x">
                <table className="zoho-table">
                  ...
                  <tbody>
                    ...
                    {!dashboard.byEstado.length && (
                      <tr>
                        <td colSpan={3}>
                          Sin empresas para los filtros seleccionados.
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            <section className="zoho-table-wrap">
              <div className="zoho-table-toolbar">
                <span>Seguimiento y medición por KAM</span>
                <span className="zoho-help-text">
                  Control comercial por vendedor: carga, avance, ganadas,
                  perdidas, vencidas y conversión.
                </span>
              </div>
              <div className="zoho-table-scroll-x">
                <table className="zoho-table kam-wide-table">
                  ...
                  <tbody>
                    ...
                    {!dashboard.byKam.length && (
                      <tr>
                        <td colSpan={11}>Sin métricas disponibles.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <section className="zoho-table-wrap">
              <div className="zoho-table-toolbar">
                <span>Empresas críticas para acción</span>
                <span className="zoho-help-text">
                  Empresas vencidas, sin contacto o marcadas para reasignar.
                </span>
              </div>
              <div className="zoho-table-scroll-x">
                <table className="zoho-table kam-wide-table">
                  ...
                  <tbody>
                    {filteredCompanies
                      .filter(
                        (row) =>
                          isOverdue(row.proxima_gestion) ||
                          (!row.fecha_ultimo_contacto && row.kam_asignado_id) ||
                          row.estado === "Reasignar",
                      )
                      .slice(0, 30)
                      .map((row) => (
                        <tr key={row.id}>
                          ...
                          <td>{row.estado || "—"}</td>
                          <td>
                            <span className={prioridadClass(row.prioridad)}>
                              {row.prioridad || "—"}
                            </span>
                          </td>
                          <td>{money(row.monto_devolucion)}</td>
                    ...
                    {!filteredCompanies.some(
                      ...
                      <tr>
                        <td colSpan={8}>
                          No hay alertas comerciales para los filtros actuales.
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        {tab === "agenda" && (
          <section className="zoho-table-wrap">
            <div className="zoho-table-toolbar">
              <span>Agenda comercial y alertas</span>
              <span className="zoho-help-text">
                Gestiones vencidas, de hoy, mañana y empresas sin primer
                contacto.
              </span>
            </div>
            <div className="zoho-table-scroll-x">
              <table className="zoho-table kam-wide-table">
                ...
                <tbody>
                  {agenda.map((row) => (
                    <tr key={row.id}>
                      <td>
                        <strong>{row.alerta || "Próxima"}</strong>
                      </td>
                      <td>{row.razon_social}</td>
                      <td>{row.rut}</td>
                      <td>{row.kam || "Sin asignar"}</td>
                      <td>{row.estado || "—"}</td>
                      <td>{row.prioridad || "—"}</td>
                      <td>{money(row.monto_devolucion)}</td>
                      <td>
                      ...
                      <td>
                        {row.fecha_ultimo_contacto
                          ? String(row.fecha_ultimo_contacto).slice(0, 10)
                          : "—"}
                      </td>
                      <td>
                        <button
                  ...
                  {!agenda.length && (
                    <tr>
                      <td colSpan={10}>Sin agenda comercial disponible.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="mandantes-layout">
            <aside className="zoho-filter-panel">
              ...
              <div className="zoho-form-grid single-column-form">
                ...
                <Field label="Objetivo monto">
                  <input
                    className="zoho-input"
                    type="number"
                    value={campaignForm.objetivo_monto}
                    onChange={(e) =>
                      setCampaignForm({
                        ...campaignForm,
                        objetivo_monto: e.target.value,
                      })
                    }
                  />
                </Field>
                ...
                <hr />
                <Field label="Campaña para importar">
                  <select
                    className="zoho-input"
                ...
                <button className="zoho-btn" onClick={exportCompanies}>
                  Exportar empresas filtradas
                </button>
                <p className="zoho-help-text">
                  Columnas aceptadas: RUT, Razón Social, Nro Empleados, Monto
                  Devolución, Rubro, Región, Contacto, Cargo, Correo, Teléfono,
                  Teléfono central y LinkedIn.
                </p>
              </div>
            </aside>
            <section className="zoho-table-wrap">
              <div className="zoho-table-toolbar">
                <span>Campañas comerciales {campaigns.length}</span>
              </div>
              <table className="zoho-table kam-wide-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  {campaigns.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td>{c.estado || "—"}</td>
                      <td>{c.empresas || 0}</td>
                      <td>{c.contactadas || 0}</td>
                      <td>{c.interesadas || 0}</td>
                      <td>{c.propuestas || 0}</td>
                      <td>{c.ganadas || 0}</td>
                      <td>{c.perdidas || 0}</td>
                      <td>{money(c.monto_potencial)}</td>
                      <td>{money(c.monto_ganado)}</td>
                    </tr>
                  ))}
                  {!campaigns.length && (
                    <tr>
                      <td colSpan={10}>Sin campañas creadas.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        {tab === "profiles" && (
          <div className="mandantes-layout">
              ...
              {canConfigure ? (
                <div className="zoho-form-grid single-column-form">
                  ...
                  <Field label="Nivel experiencia">
                    <select
                      className="zoho-input"
                      value={profileForm.nivel_experiencia}
                      onChange={(e) =>
                        setProfileForm({
                          ...profileForm,
                  ...
                  <Rating
                    label="Licitaciones"
                    value={profileForm.experiencia_licitaciones}
                    onChange={(v) =>
                      setProfileForm({
                        ...profileForm,
                        experiencia_licitaciones: v,
                      })
                    }
                  />
                  <Rating
                    label="Ventas"
                    value={profileForm.experiencia_ventas}
                    onChange={(v) =>
                      setProfileForm({ ...profileForm, experiencia_ventas: v })
                    }
                  />
                  <Rating
                    label="Recuperaciones"
                    value={profileForm.experiencia_recuperaciones}
                    onChange={(v) =>
                      setProfileForm({
                        ...profileForm,
                        experiencia_recuperaciones: v,
                      })
                    }
                  />
                  <Rating
                    label="Empresas grandes"
                    value={profileForm.experiencia_empresas_grandes}
                    onChange={(v) =>
                      setProfileForm({
                        ...profileForm,
                        experiencia_empresas_grandes: v,
                      })
                    }
                  />
                  <Rating
                    label="Empresas pequeñas"
                    value={profileForm.experiencia_empresas_pequenas}
                    onChange={(v) =>
                      setProfileForm({
                        ...profileForm,
                        experiencia_empresas_pequenas: v,
                      })
                    }
                  />
                  <Field label="Rubros fuertes">
                    <input
                      className="zoho-input"
                      placeholder="Minería, Construcción, Servicios"
                      value={profileForm.rubros_experiencia}
                      onChange={(e) =>
                        setProfileForm({
                          ...profileForm,
                          rubros_experiencia: e.target.value,
                        })
                      }
                    />
                  </Field>
                  <Field label="Regiones fuertes">
                    <input
                      className="zoho-input"
                      placeholder="RM, Antofagasta, Biobío"
                      value={profileForm.regiones_experiencia}
                      onChange={(e) =>
                        setProfileForm({
                          ...profileForm,
                          regiones_experiencia: e.target.value,
                        })
                      }
                    />
                  </Field>
                  <Field label="Capacidad máxima">
                    <input
                      className="zoho-input"
                      type="number"
                      value={profileForm.capacidad_maxima}
                      onChange={(e) =>
                        setProfileForm({
                          ...profileForm,
                  ...
                  <Field label="Tasa cierre %">
                    <input
                      className="zoho-input"
                      type="number"
                      value={profileForm.tasa_cierre}
                      onChange={(e) =>
                        setProfileForm({
                          ...profileForm,
                          tasa_cierre: e.target.value,
                        })
                      }
                    />
                  </Field>
                  <Field label="Monto cerrado histórico">
                    <input
                      className="zoho-input"
                      type="number"
                      value={profileForm.monto_cerrado_historico}
                      onChange={(e) =>
                        setProfileForm({
                          ...profileForm,
                          monto_cerrado_historico: e.target.value,
                        })
                      }
                    />
                  </Field>
                ...
                <p className="zoho-help-text">
                  Solo admin o KAM administrador pueden modificar perfiles.
                </p>
              )}
            </aside>
            ...
            <section className="zoho-table-wrap">
              <div className="zoho-table-toolbar">
                <span>Ranking KAM {profiles.length}</span>
              </div>
              <table className="zoho-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  {profiles.map((p) => (
                    <tr key={p.user_id}>
                      ...
                      <td>
                        <strong>{p.ranking_kam}</strong>
                      </td>
                      <td>
                        {p.carga_actual || 0}/{p.capacidad_maxima}
                      </td>
                      <td>{p.experiencia_licitaciones}</td>
                      <td>{p.experiencia_ventas}</td>
                      <td>{p.experiencia_recuperaciones}</td>
                      <td>{p.experiencia_empresas_grandes}</td>
                      <td>{p.experiencia_empresas_pequenas}</td>
                      <td>{csv(p.rubros_experiencia)}</td>
                      <td>{csv(p.regiones_experiencia)}</td>
                      <td>
                        {canConfigure && (
                          <button
                            className="zoho-small-btn"
                  ...
                  {!profiles.length && (
                    <tr>
                      <td colSpan={12}>Sin perfiles KAM configurados.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        {tab === "rules" && (
          <div className="mandantes-layout">
            <aside className="zoho-filter-panel">
              ...
              {canConfigure ? (
                <div className="zoho-form-grid single-column-form">
                  <Field label="Nombre regla">
                    <input
                      className="zoho-input"
                      value={ruleForm.nombre_regla}
                      onChange={(e) =>
                        setRuleForm({
                          ...ruleForm,
                          nombre_regla: e.target.value,
                        })
                      }
                    />
                  </Field>
                  <Field label="Criterio">
                    <select
                      className="zoho-input"
                      value={ruleForm.criterio}
                      onChange={(e) =>
                        setRuleForm({ ...ruleForm, criterio: e.target.value })
                      }
                    >
                      <option value="monto_devolucion">Monto devolución</option>
                      <option value="nro_empleados">Nro empleados</option>
                      <option value="rubro">Rubro</option>
                      <option value="region">Región</option>
                      <option value="tipo_oportunidad">Tipo oportunidad</option>
                      <option value="carga_actual">Carga KAM</option>
                    </select>
                  </Field>
                  <Field label="Operador">
                    <select
                      className="zoho-input"
                      value={ruleForm.operador}
                      onChange={(e) =>
                        setRuleForm({ ...ruleForm, operador: e.target.value })
                      }
                    >
                      <option>&gt;</option>
                      <option>&gt;=</option>
                  ...
                  <Field label="Valor">
                    <input
                      ...
                      onChange={(e) =>
                        setRuleForm({ ...ruleForm, valor: e.target.value })
                      }
                    />
                  </Field>
                  <Field label="Peso">
                    <input
                      ...
                      value={ruleForm.peso}
                      onChange={(e) =>
                        setRuleForm({ ...ruleForm, peso: e.target.value })
                      }
                    />
                  </Field>
                  <Field label="Acción">
                    <input
                      className="zoho-input"
                      value={ruleForm.accion}
                      onChange={(e) =>
                        setRuleForm({ ...ruleForm, accion: e.target.value })
                      }
                    />
                  </Field>
                  <button
                ...
                <p className="zoho-help-text">
                  Solo admin o KAM administrador pueden crear reglas.
                </p>
              )}
            </aside>
            <section className="zoho-table-wrap">
              <div className="zoho-table-toolbar">
                <span>Reglas comerciales {rules.length}</span>
              </div>
              <table className="zoho-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  ...
                  {!rules.length && (
                    <tr>
                      <td colSpan={7}>
                        Sin reglas configuradas. Puedes agregar reglas como
                        monto mayor a $20.000.000, rubro minería o tipo
                        licitación.
                      </td>
                    </tr>
                  )}
          ...
          <div className="kam-modal-panel kam-modal-large">
            <div className="kam-modal-header">
              <div>
                ...
                <h2>{selected ? "Editar empresa" : "Nueva empresa"}</h2>
                <p>
                  Completa los datos comerciales, asignación y contactos sin
                  salir del dashboard.
                </p>
              </div>
              <button
              ...
              <div className="zoho-form-grid kam-company-modal-grid">
                <Field label="RUT">
                  <input
                    ...
                    disabled={Boolean(selected && !canAdmin)}
                  />
                </Field>
                <Field label="Razón Social">
                  <input
                ...
                <Field label="Rubro">
                  <input
                    ...
                    value={companyForm.rubro}
                    onChange={(e) =>
                      setCompanyForm({ ...companyForm, rubro: e.target.value })
                    }
                  />
                </Field>
                <Field label="Región">
                ...
                <Field label="Correo">
                  <input
                    ...
                    value={companyForm.correo}
                    onChange={(e) =>
                      setCompanyForm({ ...companyForm, correo: e.target.value })
                    }
                  />
                </Field>
                <Field label="Nro Telefónico contacto">
              ...
              <div className="kam-contact-builder">
                ...
                {!selected && (
                  <div className="kam-draft-contact-list">
                    ...
                    {!draftContacts.length && (
                      <p className="zoho-help-text">
                        Aún no agregas contactos adicionales.
                      </p>
                    )}
                  </div>
                )}
            ...
            <div className="kam-modal-body">
              <div className="zoho-form-grid kam-filter-grid">
                <Field label="Tipo gestión">
                  ...
                  >
                    ...
                    <option>Propuesta</option>
                    <option>Seguimiento</option>
                    <option>Contacto LinkedIn</option>
                    <option>Reasignación</option>
                  </select>
                </Field>
                <Field label="Estado venta">
              ...
              <div className="kam-modal-history">
                ...
                {activities.slice(0, 5).map((a) => (
                  <div className="kam-history-item" key={a.id}>
                    <span>
